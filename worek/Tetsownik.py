...
class SingleImageInstanceAnalyzer:
    ...
    def process_image_from_array(self, img):
        if img is None:
...
def get_embedding_crop(crop, class_name):
    """Generuje embedding dla wycinka obrazu (używana przez porównanie i test z kamery)."""
    if crop.size == 0:
        return None
...
# wewnątrz klasy App dodajemy:

    def setup_comparison_tab(self, parent):
        """Zakładka 'Porównanie' – referencja + szukanie na innym obrazie/kamerze."""
        # Ramka referencji
        ref_frame = ttk.LabelFrame(parent, text="1. WZORZEC (obraz referencyjny)")
        ref_frame.pack(fill=tk.X, padx=10, pady=10)

        src_frame = ttk.Frame(ref_frame)
        src_frame.pack(pady=5)
        self.comp_ref_source = tk.StringVar(value="file")
        ttk.Radiobutton(src_frame, text="Plik", variable=self.comp_ref_source, value="file").pack(side=tk.LEFT, padx=10)
        ttk.Radiobutton(src_frame, text="Kamera", variable=self.comp_ref_source, value="camera").pack(side=tk.LEFT, padx=10)
        ttk.Button(src_frame, text="POBIERZ WZORZEC", command=self.capture_reference).pack(side=tk.LEFT, padx=20)

        self.ref_summary = tk.Text(ref_frame, height=6, width=70)
        self.ref_summary.pack(fill=tk.X, padx=5, pady=5)

        # Ramka testu
        test_frame = ttk.LabelFrame(parent, text="2. TEST (porównanie z wzorcem)")
        test_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        ttk.Label(test_frame, text="Wybierz źródło testowe i uruchom porównanie.").pack(pady=5)

        btn_frame = ttk.Frame(test_frame)
        btn_frame.pack(pady=5)
        self.comp_test_source = tk.StringVar(value="file")
        ttk.Radiobutton(btn_frame, text="Plik", variable=self.comp_test_source, value="file").pack(side=tk.LEFT, padx=10)
        ttk.Radiobutton(btn_frame, text="Kamera", variable=self.comp_test_source, value="camera").pack(side=tk.LEFT, padx=10)

        self.start_comp_btn = ttk.Button(btn_frame, text="ROZPOCZNIJ PORÓWNANIE", command=self.start_comparison_test, state=tk.DISABLED)
        self.start_comp_btn.pack(side=tk.LEFT, padx=20)
        self.stop_comp_btn = ttk.Button(btn_frame, text="ZATRZYMAJ (kamera)", command=self.stop_comparison_camera, state=tk.DISABLED)
        self.stop_comp_btn.pack(side=tk.LEFT, padx=10)

        self.test_summary = tk.Text(test_frame, height=8, width=70)
        self.test_summary.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        # Przechowujemy referencyjne detekcje
        self.reference_detections = []   # lista słowników z polami: class, embedding, id, confidence, bbox
        self.comp_camera_active = False

    def capture_reference(self):
        """Pobiera obraz referencyjny, wykrywa obiekty, nadaje ID i zapamiętuje."""
        if self.comp_ref_source.get() == "file":
            ...
            # Ukryj okno Tkintera na czas przechwytywania
            self.root.withdraw()
            ...
            cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
            cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
            print("Naciśnij SPACJĘ, aby zrobić zdjęcie referencyjne (ESC – anuluj).")
            img = None
            while True:
                ret, frame = cap.read()
                if not ret:
                    break
                cv2.imshow("Kamera - zdjęcie referencyjne", frame)
                key = cv2.waitKey(30) & 0xFF
                if key == 32:   # spacja
                    img = frame.copy()
                    break
                    ...
                    break
            cap.release()
            cv2.destroyWindow("Kamera - zdjęcie referencyjne")
            self.root.deiconify()
            if img is None:
                self.ref_summary.delete(1.0, tk.END)
                self.ref_summary.insert(tk.END, "Anulowano pobieranie wzorca.\n")
                return

        # Analizuj obraz jak w szybkiej analizie
        try:
            analyzer = SingleImageInstanceAnalyzer()
            annotated, detections = analyzer.process_image_from_array(img)   # musimy dodać metodę dla tablicy
        except Exception as e:
            messagebox.showerror("Błąd", str(e))
            return

        # Zapisz detekcje jako wzorce referencyjne
        self.reference_detections = []
        for det in detections:
            self.reference_detections.append({
                'class': det['class'],
        ...
        for d in self.reference_detections:
            summary[d['class']].append(d['id'])
        for cls, ids in sorted(summary.items()):
            self.ref_summary.insert(tk.END, f"{cls}: {len(ids)} obiektów, ID: {sorted(set(ids))}\n")
        self.ref_summary.insert(tk.END, f"\nWzorzec zapamiętany – {len(self.reference_detections)} obiektów.\n")
        self.start_comp_btn.config(state=tk.NORMAL)

        # Opcjonalnie pokaż wynik analizy (z adnotacjami)
        cv2.imshow("Wzorzec – unikalne ID", annotated)
        cv2.waitKey(0)
        cv2.destroyWindow("Wzorzec – unikalne ID")

    def start_comparison_test(self):
        """Uruchom porównanie na podstawie zapamiętanych wzorców."""
        if not self.reference_detections:
            messagebox.showwarning("Brak wzorca", "Najpierw pobierz obraz referencyjny.")
            return

        test_source = self.comp_test_source.get()
        if test_source == "file":
            filepath = filedialog.askopenfilename(
            ...
            if img is None:
                ...
                return
            self._process_test_image(img)
        else:   # camera
            self.start_comp_btn.config(state=tk.DISABLED)
            self.stop_comp_btn.config(state=tk.NORMAL)
            self.comp_camera_active = True
            # Uruchom kamerę w osobnym wątku, aby nie blokować GUI
            import threading
            self.camera_thread = threading.Thread(target=self._run_camera_comparison)
            self.camera_thread.daemon = True
            self.camera_thread.start()

    def stop_comparison_camera(self):
        """Zatrzymaj test z kamery."""
        self.comp_camera_active = False
        self.stop_comp_btn.config(state=tk.DISABLED)
        self.start_comp_btn.config(state=tk.NORMAL)

    def _process_test_image(self, img):
        """Przetwarza obraz testowy i dopasowuje do wzorców."""
        # Wykryj wszystkie obiekty
        results = model_yolo(img, conf=0.25, imgsz=640, verbose=False)
        detections = []
        for result in results:
            if result.boxes is not None:
                for box in result.boxes:
                    ...
                    crop = img[y1:y2, x1:x2]
                    emb = get_embedding_crop(crop, cls_name)   # zdefiniowana wcześniej funkcja
                    if emb is not None:
                        detections.append({
                            'bbox': (x1,y1,x2,y2),
                            'class': cls_name,
        ...
        for det in detections:
            cls = det['class']
            emb = det['embedding']
            best_id, best_sim = self._match_to_reference(emb, cls)
            if best_id is not None:
                color = (0,255,0)
                label = f"{cls}_{best_id} (sim:{best_sim:.2f})"
                test_summary_lines.append(f"[{cls}] -> {cls}_{best_id} (podobieństwo: {best_sim:.3f})")
            else:
                color = (0,0,255)
                label = f"UNKNOWN {cls}"
                test_summary_lines.append(f"[{cls}] -> NIEZNANY")
        ...
        out_path = f"porownanie_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
        cv2.imwrite(out_path, annotated)
        cv2.imshow("Porównanie – wynik", annotated)

        # Wyświetl podsumowanie
        self.test_summary.delete(1.0, tk.END)
        self.test_summary.insert(tk.END, f"Zapisano: {out_path}\n\n")
        for line in test_summary_lines:
            self.test_summary.insert(tk.END, line + "\n")

    def _run_camera_comparison(self):
        """Pętla kamery dla trybu porównania."""
        cap = cv2.VideoCapture(CAMERA_ID)
        if not cap.isOpened():
            self.root.after(0, lambda: messagebox.showerror("Błąd", "Nie można otworzyć kamery"))
            self.comp_camera_active = False
            return
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
        print("Kamera testowa (porównanie) – naciśnij 'q', aby zakończyć.")
        while self.comp_camera_active:
            ret, frame = cap.read()
            if not ret:
                break
            # Dla uproszczenia korzystamy z tej samej metody co dla pliku
            # (ale może być wolno; w razie potrzeby można zoptymalizować)
            annotated, summary_lines = self._process_frame_for_comparison(frame)
            cv2.imshow("Kamera – porównanie", annotated)
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break
        cap.release()
        cv2.destroyWindow("Kamera – porównanie")
        # Przywróć stan przycisków
        self.root.after(0, self.stop_comparison_camera)

    def _process_frame_for_comparison(self, frame):
        """Zwraca obraz z adnotacjami i listę linii podsumowania (dla kamery)."""
        ...
        for result in results:
            if result.boxes is not None:
                for box in result.boxes:
                    ...
                    if emb is None:
                        continue
                    best_id, best_sim = self._match_to_reference(emb, cls_name)
                    if best_id is not None:
                        color = (0,255,0)
                        label = f"{cls_name}_{best_id} ({best_sim:.2f})"
                        summary.append(f"{cls_name}_{best_id} (sim:{best_sim:.2f})")
                    else:
                        color = (0,0,255)
                        label = f"UNKNOWN {cls_name}"
                        summary.append(f"UNKNOWN {cls_name}")
                    cv2.rectangle(annotated, (x1,y1), (x2,y2), color, 2)
                    (tw,th), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 2)
                    cv2.rectangle(annotated, (x1,y1-th-5), (x1+tw,y1), color, -1)
                    cv2.putText(annotated, label, (x1,y1-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 2)
    ...
    def _match_to_reference(self, embedding, class_name):
        """Szuka najlepszego dopasowania w self.reference_detections."""
        candidates = [ref for ref in self.reference_detections if ref['class'] == class_name]
        if not candidates:
            return None, 0.0
        threshold = 0.75 if class_name == 'person' else 0.85
        best_id = None
        ...
        for ref in candidates:
            sim = 1 - cosine(embedding, ref['embedding'])
